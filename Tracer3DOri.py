  ...
  for j in yrng:
    for k in zrng:
      ...
      if ratio>0.0:
         ...
         c0=eigvec.get(minind,0)
         c1=eigvec.get(minind,1)
         c2=eigvec.get(minind,2)
         maxind=maxindex([abs(c0),abs(c1),abs(c2)])
         voxval=image.getVoxel(x,y,z)
         if voxval>77:
           if maxind==0:
              image.setVoxel(x,y,z,190)
           elif maxind==1:
              image.setVoxel(x,y,z,255)
           elif maxind==2:
              image.setVoxel(x,y,z,120)     
         #PRoi.addPoint(x,y)  
         #x1=float(x+c0*scaling)
         #y1=float(y+c2*scaling)
         #z1=float(z+c1*scaling)
